...
gchar *gdial_util_xml_encode(const gchar *src) {
 ...
 /*
  * Preallocate twice the size
  */
  GString *dst = g_string_new_len('\0', 2*(length+1));

  while (*src) {
    switch (*src) {
      case '&':
        dst = g_string_append(dst, "&amp;");
        break;
      case '\"':
        dst = g_string_append(dst, "&quot;");
        break;
...
gchar *gdial_util_url_decode_xml_encode(const gchar *str) {
  gchar *decoded = soup_uri_decode(str);
  gchar *encoded = gdial_util_xml_encode(decoded);
  g_free(decoded);
  return encoded;
}
